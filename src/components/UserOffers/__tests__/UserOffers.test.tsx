...
describe('UserOffers Component', () => {
    ...
    it('displays an error message when the API call fails', async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText(/Nie udało się pobrać ogłoszeń/)
            ).toBeInTheDocument()
        )
    })
    ...
    it('renders empty-state when API returns zero offers', async () => {
        ...
        await waitFor(() =>
            expect(
                screen.getByText(/nie masz żadnych aktywnych ogłoszeń/i)
            ).toBeInTheDocument()
        )

        fireEvent.click(
            screen.getByRole('button', {name: /Dodaj nowe ogłoszenie/})
        )
    ...
    it('renders real offers, handles image error, navigation, edit and delete', async () => {
        ...
        await waitFor(() =>
            expect(screen.queryByText('First Car')).not.toBeInTheDocument()
        )
    })
})
