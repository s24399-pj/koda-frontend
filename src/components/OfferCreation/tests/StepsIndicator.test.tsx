...
describe('StepsIndicator Component', () => {
  ...
  it('applies "active" class to steps up to and including activeStep', () => {
    const activeIndex = 2;
    const { container } = render(<StepsIndicator steps={steps} activeStep={activeIndex} />);
  ...
  it('handles activeStep greater than last index (all steps active)', () => {
    const { container } = render(<StepsIndicator steps={steps} activeStep={steps.length + 5} />);
    container.querySelectorAll('.step').forEach(el => {
      expect(el).toHaveClass('active');
    });
  });
  ...
  it('matches snapshot', () => {
    const { asFragment } = render(<StepsIndicator steps={steps} activeStep={2} />);
