...
vi.mock('formik', async () => {
  ...
  return {
    ...
    Field: ({
      ...
      const Component =
        as || (type === 'checkbox' ? 'input' : type === 'number' ? 'input' : 'input');
...
vi.mock('../ImageUpload', () => ({
  default: () => <div data-testid="image-upload">ImageUpload Component</div>,
}));
...
describe('BasicInfoStep', () => {
  ...
  test('displays correct placeholders', () => {
    ...
    expect(screen.getByTestId('field-title')).toHaveAttribute(
      'placeholder',
      'Np. BMW Seria 3 318i, 2020, Salon Polska'
    );
    expect(screen.getByTestId('field-description')).toHaveAttribute(
      'placeholder',
      'Opisz dokładnie swój samochód. Wymień jego zalety, historię, stan techniczny i wszystko co może zainteresować potencjalnych kupujących.'
    );
