...
import { ApiOffer, OfferData } from '../../../types/offerTypes';
...
vi.mock('../../../components/LikeButton/LikeButton', () => ({
    default: ({ offerId, initialLiked }: { offerId: string, initialLiked: boolean }) => (
        <button data-testid={`like-button-${offerId}`}>
            {initialLiked ? '❤️' : '🤍'}
        </button>
    )
}));
...
describe('SellerOffers Component', () => {
    ...
    test('handles image loading errors', async () => {
        ...
        expect(image.src).toBe('http://localhost/default-car.png');
        expect(consoleSpy).toHaveBeenCalledWith("Image loading error in seller's offer:", expect.any(String));
    ...
    test('shows no offers message when seller has no offers', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText('Ten sprzedający nie ma obecnie żadnych aktywnych ogłoszeń.')).toBeInTheDocument();
        });
    });
    ...
    test('handles API error gracefully', async () => {
        ...
        await waitFor(() => {
            expect(screen.getByText("Failed to fetch seller's offers. Please try again later.")).toBeInTheDocument();
        });
    });
    ...
    test('truncates long offer titles', async () => {
        const longTitleOffer: ApiOffer = {
            ...mockApiOffers[0],
            title: 'This is a very very very very long title that should be truncated when displayed'
        };
        ...
        await waitFor(() => {
            // The title should be truncated to 50 characters
            const truncatedTitle = 'This is a very very very very long title that sho...';
    ...
    test('handles offers count pluralization correctly', async () => {
        ...
        const fiveOffers = Array(5).fill(null).map((_, i) => ({ ...mockApiOffers[0], id: `${i + 1}` }));
        mockGetOffersBySeller.mockResolvedValue({
            content: fiveOffers,
            totalPages: 1,
            number: 0,
    ...
    test('maps API response to component format correctly', async () => {
        const incompleteApiOffer: ApiOffer = {
            id: '3',
        ...
        await waitFor(() => {
            expect(screen.getByText('Incomplete Car')).toBeInTheDocument();
            expect(screen.getByText('100 000 zł')).toBeInTheDocument(); // Default currency
            expect(screen.getByText('Unknown')).toBeInTheDocument(); // Default brand/model
        });
    });
});