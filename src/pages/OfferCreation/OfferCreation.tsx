...
import { CarEquipment, CreateOfferCommand, OfferFormValues } from '../../types/offer/OfferTypes';
...
const validationSchemas = [
  ...
  Yup.object({
    ...
    year: Yup.number()
      ...
      .max(
        ...
        `Rok produkcji nie może być większy niż ${new Date().getFullYear()}`
      )
    ...
    condition: Yup.string().required('Stan pojazdu jest wymagany'),
    color: Yup.string()
    ...
    bodyType: Yup.string().required('Typ nadwozia jest wymagany'),
    driveType: Yup.string().required('Napęd jest wymagany'),
    displacement: Yup.string().required('Pojemność silnika jest wymagana'),
  ...
  Yup.object({
    location: Yup.string().nullable().max(100, 'Lokalizacja nie może przekraczać 100 znaków'),
    expirationDate: Yup.date()
      .nullable()
      .min(
        new Date(new Date().setDate(new Date().getDate() + 1)),
        'Data wygaśnięcia musi być w przyszłości (minimum dzień później)'
      )
      ...
      >
        {(formik: FormikProps<OfferFormValues>) => (
          <Form className="offer-creation-form">
            {activeStep === 0 && <BasicInfoStep formik={formik} onNext={handleNextStep} />}
