...
test.describe('UI Chaos Testing', () => {
    ...
    test('random clicks on interactive elements', async ({ page }) => {
        ...
        const clickableElements = await page.locator('button, a, input[type="submit"], [role="button"]').all();
    ...
    test('drag and drop chaos', async ({ page }) => {
        ...
        if (draggableElements.length > 0) {
            ...
            while (Date.now() < endTime && draggableElements.length > 1) {
                const sourceElement = draggableElements[Math.floor(Math.random() * draggableElements.length)];
                const targetElement = draggableElements[Math.floor(Math.random() * draggableElements.length)];
